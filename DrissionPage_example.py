...
if platform.system() == "Linux":
    ...
    _pw_chromes = glob.glob(os.path.expanduser("~/.cache/ms-playwright/chromium-*/chrome-linux*/chrome"))
    if _pw_chromes:
...
def push_sso_to_api(new_tokens: list[str]) -> None:
    ...
    conf = load_config()
    if not conf:
        print("[Warn] 读取 config.json 失败，跳过推送")
        return
    ...
    pool = str(api_conf.get("pool", "basic")).strip().lower()

    if not endpoint or not api_token:
        return

    api_verify_tls = bool(api_conf.get("verify_tls", True))

    if endpoint.endswith("/tokens/add"):
        add_tokens_url = endpoint
        save_tokens_url = endpoint.removesuffix("/tokens/add") + "/tokens"
    elif endpoint.endswith("/tokens"):
        save_tokens_url = endpoint
        add_tokens_url = endpoint + "/add"
    ...
    if endpoint.endswith("/api/accounts"):
        ...
        request_url = save_tokens_url
        payload = {pool: tokens_to_push}
    ...
    try:
        ...
        if resp.status_code == 200:
            print(f"[*] SSO token 已推送到 API（共 {len(tokens_to_push)} 个）: {request_url}")
        else:
...
def load_run_settings() -> tuple[int, float, float]:
    # 从 config.json 读取执行轮数与轮次间延迟抖动。
    conf = load_config()
    ...
    delay_min_seconds = run_conf.get("delay_min_seconds", 2)
    delay_max_seconds = run_conf.get("delay_max_seconds", 4)

    try:
        delay_min_seconds = max(0.0, float(delay_min_seconds))
    except (TypeError, ValueError):
        delay_min_seconds = 2.0

    try:
        delay_max_seconds = max(delay_min_seconds, float(delay_max_seconds))
    except (TypeError, ValueError):
        delay_max_seconds = max(delay_min_seconds, 4.0)

    return count, delay_min_seconds, delay_max_seconds
...
def main():
    ...
    config_count, delay_min_seconds, delay_max_seconds = load_run_settings()
    ...
    try:
        ...
        while True:
            ...
            if args.count == 0 or current_round < args.count:
                round_delay_seconds = random.uniform(delay_min_seconds, delay_max_seconds)
                print(f"[*] 等待 {round_delay_seconds:.1f} 秒后进入下一轮")
